...
void CL_GlobalServers_f(void) {
	...
	if (adr[adrNum].type == NA_IP6 || adr[adrNum].type == NA_MULTICAST6) {
		...
		// in Urban Terror, the gamename sent to the server in the challenge is different from the one sent to the master. why?
	...
	Com_DPrintf("The command we're sending is: %s\n", command);
}
