...
void RB_ToneMap(FBO_t *hdrFbo, ivec4_t hdrBox, FBO_t *ldrFbo, ivec4_t ldrBox, int autoExposure)
{
	...
	if (autoExposure)
	{
		...
		if (glRefConfig.textureFloat)
			...
			color[3] = 0.1f;
		// Pass null instead of a 0x0 box
	...
	// tonemap
	{
		float ev = r_cameraExposure->value - (autoExposure ? 1.0f : 0.0f);
		color[0] = color[1] = color[2] = exp2f(ev);
...
/* resolution-independent scaling:
	   convert "strength" in pixels to normalized texel offsets */

	float xmul = 0.0f;
	float ymul = 0.0f;

	if (horizontal) {
		xmul = strength / (float)srcFbo->width;
		
	}
	else {
		ymul = strength / (float)srcFbo->height;
