...
void R_BuildCloudData(shaderCommands_t* input)
{
	...
	assert(shader && shader->isSky);

	// Touch nothing for cube only skies
	if (shader->sky.cloudHeight <= 0) {
		return;
	}

	// Compute spec-correct half-texel clamps from the uploaded face size.
	// Texel centers are at (i+0.5)/N; keeping the LINEAR footprint inside
	// the image implies clamping to [0.5/N, 1-0.5/N]. We derive N from the
	// actual upload dimensions so picmip/skymip/NPOT are handled.
	
	int faceW = 512, faceH = 512;
	for (int f = 0; f < 6; ++f) {
		image_t * img = shader->sky.outerbox[f];
		if (img && img != tr.defaultImage && img->uploadWidth > 0 && img->uploadHeight > 0) {
			faceW = img->uploadWidth;
			faceH = img->uploadHeight;
			break;
			
		}
	}
	const int dim = (faceW > faceH) ? faceW : faceH;
	const float halfTexel = 0.5f / (float)dim; // = 1/(2N)

	sky_min = halfTexel;
	sky_max = 1.0f - halfTexel;
...
void RB_StageIteratorSky(void) {
	...
	// Flush before tess overflows
	if (tess.numVertexes + 1 > SHADER_MAX_VERTEXES ||
		tess.numIndexes >= SHADER_MAX_INDEXES) {
		RB_EndSurface();
